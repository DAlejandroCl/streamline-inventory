/* ============================================================
   ...
   Tests que verifican comportamiento SIN sesión usan
...
test.describe("E2E — Authentication", () => {
  ...
  test("login con credenciales válidas redirige al dashboard", async ({ browser }) => {
  ...
  test("login con password incorrecto muestra mensaje de error", async ({ browser }) => {
    ...
    // El Toast tiene role="alert". hasText exacto evita strict mode violation
    // con texto estático "Demo credentials are pre-filled above." en la página.
    await expect(
  ...
  test("login con email inexistente muestra error de validación", async ({ browser }) => {
    const ctx  = await browser.newContext({ storageState: { cookies: [], origins: [] } });
    const page = await ctx.newPage();
  ...
  /* ---- Protección de rutas (sin sesión) -----------------
     Context limpio para anular el storageState global de admin.
     -------------------------------------------------------- */
  ...
  test("el botón de login muestra estado loading durante el submit", async ({ browser }) => {
    const ctx  = await browser.newContext({ storageState: { cookies: [], origins: [] } });
    const page = await ctx.newPage();
