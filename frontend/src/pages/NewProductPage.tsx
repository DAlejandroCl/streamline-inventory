  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <aside className="bg-(--color-surface) rounded-2xl p-6 shadow-card border border-(--color-border)/40 space-y-6">
          <div>
          ...
          <ul className="space-y-4">
            {TIPS.map(({ icon: Icon, text }) => (
              <li key={text} className="flex items-start gap-3">
                ...
                <p className="text-sm text-(--color-text-secondary) leading-relaxed">{text}</p>
              </li>
            ))}
          </ul>
