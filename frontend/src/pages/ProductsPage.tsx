...
import { Plus, Download, PackageX, Filter } from "lucide-react";
...
export default function ProductsPage() {
  ...
  const filtered =
    filter === "available"
      ? products.filter((p) => p.availability)
      : filter === "out"
      ? products.filter((p) => !p.availability)
      : products;
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-3">
            <Button variant="secondary" icon={Download} size="lg">Export CSV</Button>
            <Link to="/products/new">
              <Button icon={Plus} size="lg">Add Product</Button>
            </Link>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total Items",   value: total.toString(),          color: "text-[var(--color-primary)]" },
          { label: "Available",     value: available.toString(),      color: "text-[var(--color-secondary)]" },
          { label: "Out of Stock",  value: outOfStock.toString(),     color: outOfStock > 0 ? "text-[var(--color-error)]" : "text-[var(--color-text-muted)]" },
          { label: "Total Value",   value: formatCurrency(totalValue), color: "text-[var(--color-text-primary)]" },
        ].map((s) => (
          <div
            key={s.label}
      ...
      {/* FILTER TABS */}
      <div className="flex items-center gap-3">
        <Filter size={14} className="text-[var(--color-text-muted)" strokeWidth={2} />
